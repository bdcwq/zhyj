    ...
    <view v-else>
      ...
      <view class="info-card">
        ...
        <view class="info-grid">
          ...
          <view class="info-item">
            <text class="info-label">监测次数</text>
            <text class="info-value">{{ stats?.totalCount ?? 0 }} 次</text>
          </view>
          <view class="info-item">
            <text class="info-label">最近评分</text>
            <view class="latest-score-wrap">
              <text
                class="info-value latest-score"
                :class="{
                  'score-high': (stats?.latestScore ?? 0) >= 80,
                  'score-mid': (stats?.latestScore ?? 0) >= 60 && (stats?.latestScore ?? 0) < 80,
                  'score-low': (stats?.latestScore ?? 0) < 60,
                }"
              >{{ stats?.latestScore ?? '-' }}</text>
            </view>
          </view>
          <view class="info-item">
            <text class="info-label">平均评分</text>
            <text class="info-value">{{ stats?.averageScore ?? '-' }}</text>
          </view>
        </view>
      </view>
      ...
      <view class="chart-card">
        <text class="card-title">体质评分趋势</text>
        <canvas
        ...
        <view v-else class="chart-placeholder">
          <text class="placeholder-text">{{ records.length === 1 ? '至少需要2条记录才能显示趋势图' : '暂无监测数据' }}</text>
        </view>
      </view>

      <!-- Recent records -->
      <view v-if="records.length > 0" class="records-section">
        <text class="card-title">近期监测记录</text>
        <view
        ...
        >
          <view class="record-left">
            ...
            <text class="record-type">{{ record.constitutionType || '综合评估' }}</text>
          </view>
          <view
            class="record-score-badge"
            :class="{
              'badge-high': record.score >= 80,
              'badge-mid': record.score >= 60 && record.score < 80,
              'badge-low': record.score < 60,
            }"
          >
      ...
      <view v-else class="empty-section">
        ...
        <text class="empty-desc">完成体质监测后，数据将在此处展示</text>
      </view>
    </view>
  </view>
</template>
...
const displayName = computed(() => authStore.residentName || '未知')
...
onMounted(async () => {
  ...
  if (!residentId) {
    console.warn('[health] No residentId found, cannot fetch data')
    loading.value = false
    return
  }

  try {
    console.log('[health] Fetching health data for resident:', residentId)
    ...
    if (res.success && res.data) {
      ...
      console.log('[health] Loaded', records.value.length, 'records, stats:', stats.value)
...
function drawChart() {
  ...
  query
    ...
    .exec(() => {
      if (!canvasNode) {
        console.warn('[health] Canvas node not found')
        return
      }
